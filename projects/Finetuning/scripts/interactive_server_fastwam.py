...
"""Interactive LIBERO demo driven by the FastWAM world-action model - notebook 3.

Reuses the model-agnostic `sim_libero` harness shipped in the image (the SAME closed-loop /
chunk-replay control loop the MolmoAct2 interactive demo uses) but swaps in the FastWAM policy
via POLICY_FACTORY=fastwam_libero_policy:build_policy. FastWAM takes the DEFAULT "fast route"
(`infer_action`): from the current camera view + proprio + your instruction it plans a short
action chunk, the sim executes it, then it re-plans - the same receding-horizon loop as the
shipped closed-loop eval.

Command-driven UX: the sim sits IDLE on a scene until you send an instruction; then it resets the
env + policy and runs that one task to completion (or until Stop), streaming the composed
agentview|wrist view. Pick any shipped LIBERO task from the environment dropdown. The last run's
debug video stays on screen with a download link.

Proxy-hardened for JupyterHub: single-frame `/frame` polling (no multipart MJPEG that stalls
through jupyter-server-proxy) and RELATIVE URLs everywhere, so it embeds via
{JUPYTERHUB_SERVICE_PREFIX}/proxy/PORT/. The engine runs in a guarded thread so any load/eval
failure surfaces as mode=error in /status instead of hanging on "loading" forever.

Env: SUITE (libero_object), TASK_ID (0), SEED (1000), PORT (8080), VIEW_RES (720),
VIDEO_RES (720), RENDER_RES (512), MAX_STEPS (0=suite default), OUT_DIR (/outputs), plus the
FastWAM policy knobs read by the adapter (CKPT, DATASET_STATS, MIXED_PRECISION, NUM_INFERENCE_STEPS,
REPLAN_STEPS, NUM_STEPS_WAIT, FASTWAM_REPO). Open http://localhost:PORT or embed via the proxy.
"""
import json
import os
import threading
...
STATE = {
    ...
    "step": 0, "plans": 0, "infer_ms": 0.0, "success": False,
...
def _timed_policy(policy):
    """Wrap predict_action_chunk so the UI can show live plan latency (the fast-route
    infer_action time) and a running plan counter, without touching the harness loop."""
    _orig = policy.predict_action_chunk

    def _wrapped(obs, instruction):
        t0 = time.perf_counter()
        chunk = _orig(obs, instruction)
        dt = (time.perf_counter() - t0) * 1000.0
        with LOCK:
            STATE["infer_ms"] = round(dt, 0)
            STATE["plans"] += 1
        return chunk

    policy.predict_action_chunk = _wrapped
    return policy


def engine_thread():
    os.makedirs(os.path.join(OUT_DIR, "interactive_fastwam"), exist_ok=True)
    with LOCK:
        STATE["status"] = "loading FastWAM policy (T5 + VAE + DiT; first run JIT-compiles kernels) ..."
    policy = _timed_policy(load_policy())

    def show_idle(sc, keep_video=False):
        obs = sc.reset()
        upd = dict(mode="idle", status="idle - send an instruction", step=0, plans=0,
    ...
    obs0 = show_idle(scene)

    # Warm up the fast route NOW: the first infer_action pays a one-time ROCm HIP kernel-JIT +
    # attention autotune (tens of seconds). Do it before going idle so the first real command
    # isn't a long silent stall.
    with LOCK:
        STATE["status"] = "warming up GPU kernels (one-time JIT) ..."
    try:
        policy.warmup(obs0, scene.description or "pick up the object")
    except Exception as e:  # noqa: BLE001 - warmup is best-effort
        print("warmup failed (continuing):", e, flush=True)
    with LOCK:
        STATE["plans"] = 0
        STATE["infer_ms"] = 0.0
        STATE["status"] = "idle - send an instruction"

    def run_command(sc, instruction, max_steps):
        with LOCK:
            STATE.update(mode="running", instruction=instruction, step=0, plans=0, success=False,
        ...
        if frames:
            ...
            name = f"interactive_fastwam/{ts}_{sc.suite}_{sc.task_id}_{'ok' if success else 'run'}.mp4"
...
<p class=sub>Wan2.2-TI2V-5B world-action model on the fast route: from the current view it plans an action chunk (infer_action), the sim executes it, then it re-plans. Pick a task, type an instruction, press Send.</p>
<img id="sim" alt="sim">
<div class=row>
...
async function poll(){
 try{const s=await(await fetch('status')).json();
  document.getElementById('meta').textContent='['+s.mode+'] '+s.status+' | step '+s.step+' | plans '+s.plans+' | last plan '+s.infer_ms+' ms'+(s.instruction?' | running: "'+s.instruction+'"':'');
...
def _engine_guard():
    """Run the engine; surface any load/eval error in STATE so the UI shows 'error' instead of
    hanging on 'loading' forever (important for the workshop)."""
    try:
        engine_thread()
    except Exception as e:  # noqa: BLE001
        import traceback
...
def main():
    ...
    print(f"FastWAM interactive demo on http://0.0.0.0:{PORT}  "
